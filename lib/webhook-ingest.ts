...
import { parseWebhookSignal } from "@/lib/webhook-schema";

/**
 * Shared POST handler for every incoming signal source (TrendSpider Strategy
 * Bots, a TradingView Pine `alert()` call, a manual curl test, ...) — used by
 * both `/api/webhooks/signal` (canonical) and the `/api/webhooks/trendspider`
 * back-compat alias kept for TrendSpider bots already configured with that
 * URL. `defaultSource` labels the `WebhookEvent` row when the payload itself
 * doesn't set an explicit `"source"` field.
 */
export async function ingestWebhookSignal(
  request: Request,
  ...
  const parsed = parseWebhookSignal(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
        ok: false,
  ...
  const event = await prisma.webhookEvent.create({
    data: {
      source: signal.source ?? defaultSource,
  ...
  // smoke test in docs/webhooks.md keeps working without it.
  if (isQstashConfigured()) {
    try {
      ...
      // any other execution failure, but still ack the webhook so the
      // sender doesn't retry-storm us on top of the outage.
