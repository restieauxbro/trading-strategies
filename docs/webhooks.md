...
This app accepts HTTP POSTs from any webhook-capable signal source — TrendSpider Strategy Bots, a TradingView Pine `alert()` call, or a manual curl test — and places **paper** US share **limit** orders via Tiger Brokers.

## Endpoint

```text
POST https://<your-vercel-domain>/api/webhooks/signal?token=<WEBHOOK_TOKEN>
```

- Auth is the shared `WEBHOOK_TOKEN` query param (most alerting platforms can't set reliable custom headers). Header `x-webhook-token` also accepted.
- Body must be valid JSON.
- The sender typically times out in a few seconds; the route acknowledges quickly (always — it accepts an effectively unlimited number of simultaneous POSTs, e.g. a whole watchlist rotating at market close) and finishes Tiger placement afterward, off the request path.

**Back-compat alias:** `POST /api/webhooks/trendspider?token=...` is the same handler (identical schema/behaviour) kept for TrendSpider bots already configured with that URL. Point any **new** integration at `/api/webhooks/signal` instead.

## Concurrency / rate limiting

There's no limit on how many signals the endpoint itself can accept at once, from any source. But Tiger's own account-level API has a hard 60-second rolling window (120/min for orders + quotes, 60/min for assets/positions — see `.agents/skills/tigeropen/references/quickstart.md`), and the portfolio manager (`lib/portfolio.ts`) reads a shared account snapshot (cash, positions) that two signals executing at the same instant would race on — e.g. both trimming the same weakest position, or collectively overspending past total equity.

To avoid both problems, every accepted signal is queued through **Upstash QStash** (`lib/qstash.ts`) with `flowControl: { parallelism: 1, rate: WEBHOOK_QUEUE_RATE_PER_MINUTE }` — signals execute strictly one-at-a-time, draining at a controlled rate (default 20/min) instead of all firing concurrently, **regardless of which source they came from**. Each queued signal is delivered to `POST /api/webhooks/signal/execute` (QStash-signature-verified), which atomically claims the `WebhookEvent` row (`PENDING` → `PROCESSING`) before running it, so a QStash retry/duplicate delivery can't double-execute the same signal.
...
Every source sends only the symbol and direction — **quantity and price are resolved server-side**, not read from the payload.

| Field | Required | Notes |
| --- | --- | --- |
| `symbol` or `ticker` | Yes | Use `%bot_symbol%` (TrendSpider) or `syminfo.ticker` (Pine) |
| `action` | Yes | `buy` or `sell` |
| `source` | No | Free text label for the dashboard event log, e.g. `"tradingview"`. Defaults to whichever route the signal hit (`"signal"` or `"trendspider"`) if omitted. |
| `bot_name` | No | `%bot_name%` (TrendSpider) or a fixed string (Pine) |
| `timeframe` | No | `%bot_timeframe%` (TrendSpider) or `timeframe.period` (Pine) |
...
## TrendSpider Strategy Bot templates
...
Paste the entry JSON into the bot **Entry** webhook body and the exit JSON into the **Exit** webhook body. Point both URLs at `/api/webhooks/signal?token=...` (or the `/trendspider` alias for bots already configured with it).
...
A Pine strategy can fire this same endpoint directly via the `alert()` function — see `pinescript/ema-cross-webhook-alert.pine` for a full working template (fast/slow EMA cross example; swap the entry/exit conditions for your own logic).

For an end-to-end pipeline smoke test (TradingView alert → webhook → QStash → Tiger paper order) without waiting on a real setup, use `pinescript/noisy-webhook-test.pine` instead — it has no price logic at all and just alternates buy/sell alerts every bar close (once a day, on a Daily chart).
...
buildPayload(action, comment) =>
    '{"symbol":"'   + syminfo.ticker    +
     '","action":"' + action            +
     '","source":"tradingview"'         +
     ',"bot_name":"pine-ema-cross"'     +
     '","timeframe":"' + timeframe.period +
     '","comment":"' + comment + '"}'

if longCondition
    alert(buildPayload("buy", "entry"), alert.freq_once_per_bar_close)

if exitCondition
    alert(buildPayload("sell", "exit"), alert.freq_once_per_bar_close)
```

Setup, one time per script:

1. Add the script to a chart in the Pine Editor.
2. Create **one** alert on it: condition = "Any alert() function call", trigger = "Once Per Bar Close".
3. Under Notifications, enable **Webhook URL** and paste `https://<your-domain>/api/webhooks/signal?token=<WEBHOOK_TOKEN>` (requires a paid TradingView plan — Essential or above).
4. Leave the alert's Message box untouched — each `alert()` call supplies its own JSON body.

Unlike TrendSpider (one alert per Entry/Exit webhook body), a single Pine alert handles both directions since each `alert()` call carries its own `action`.
...
```bash
curl -X POST "http://localhost:3000/api/webhooks/signal?token=$WEBHOOK_TOKEN" \
